...
module0::module0(QLabel *parent, QString PATH) : QLabel(parent),PATH(PATH)
{
    ...
    b1->move(width()/5-b1->width()/2,height()/2-y0);
    ...
    b3 = new picButton2(this,0,PATH+"button3.png",PATH+"button3on.png","2");
    b3->move(3*width()/5-b3->width()/2,height()/2-y0);
    connect(b3,SIGNAL(clicked(QString)),this,SIGNAL(goModule3()));
    connect(b3,SIGNAL(clicked(QString)),this,SLOT(disableButtons()));
    b3->show();

    b4 = new picButton2(this,0,PATH+"button4.png",PATH+"button4on.png","2");
    b4->move(4*width()/5-b4->width()/2,height()/2-y0);
